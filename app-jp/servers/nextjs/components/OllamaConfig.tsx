  ...
  const fetchOllamaModels = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Ollamaモデルの取得に失敗しました');
        setOllamaModels([]);
        toast.error('Ollamaモデルの取得に失敗しました');
      }
    } catch (error) {
      console.error('Error fetching Ollama models:', error);
      toast.error('Ollamaモデルの取得中にエラーが発生しました');
      setOllamaModels([]);
    } finally {
        ...
        {useCustomUrl && (
          <div className="mb-4">
            ...
            <p className="mt-2 text-sm text-gray-500 flex items-center gap-2">
              ...
              カスタムOllamaインスタンスを使用している場合は変更してください
            </p>
          </div>
        )}
      </div>
      ...
      <div className="mb-4">
          ...
          {ollamaModelsLoading ? (
            <div className="w-full h-12 px-4 py-4 border border-gray-300 rounded-lg bg-gray-50 flex items-center justify-center">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-600">モデルを読み込み中...</span>
              </div>
            </div>
        ...
        {(!ollamaModels || ollamaModels.length === 0) && !ollamaModelsLoading && (
          <p className="mt-2 text-sm text-gray-500">
            利用可能なモデルがありません。Ollama接続を確認してください。
          </p>
        )}
      </div>
    </div>

  ...
  const fetchCustomModels = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('カスタムモデルの取得に失敗しました');
        setCustomModels([]);
        setCustomModelsChecked(true);
        toast.error('カスタムモデルの取得に失敗しました');
      }
    } catch (error) {
      console.error('Error fetching custom models:', error);
      toast.error('カスタムモデルの取得中にエラーが発生しました');
      setCustomModels([]);
      setCustomModelsChecked(true);
    } finally {
    ...
    <div className="space-y-6">
      ...
      {customModelsChecked && customModels.length === 0 && (
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          <p className="text-sm text-yellow-800">
            モデルが見つかりません。APIキーが有効でモデルにアクセスできることを確認してください。
          </p>
        </div>
      )}
        ...
        <div className="mb-4">
          <div className="mb-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
            <p className="text-sm text-amber-800">
              <strong>重要：</strong> 関数呼び出し機能（ツールコール）またはJSONスキーマサポートを持つモデルのみ動作します。
            </p>
          </div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            モデルを選択
      ...
      <div>
        ...
        <p className="mt-2 text-sm text-gray-500 flex items-center gap-2">
          ...
          有効にすると、構造化出力にJSONスキーマの代わりにツールコールが使用されます。
        </p>
      </div>
      {/* 思考を無効化 Toggle */}
      <div>

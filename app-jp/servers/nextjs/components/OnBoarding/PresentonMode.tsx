    ...
    const fetchAvailableModels = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setAvailableModels([]);
                setModelsChecked(true);
                toast.error(`${LLM_PROVIDERS[llmConfig.LLM!]?.label} モデルの取得に失敗しました`);
            }
        } catch (error) {
            console.error('Error fetching models:', error);
            toast.error('モデルの取得中にエラーが発生しました');
            setAvailableModels([]);
            setModelsChecked(true);
        } finally {
    ...
    return (
        <div className='w-full max-w-[640px] font-syne'>
            ...
            <div className='mb-[54px]'>
                ...
                <h2 className='mb-4 text-black text-[26px] font-normal font-unbounded '>コンテンツプロバイダーを選択</h2>
                <p className='text-[#000000CC] text-xl font-normal font-syne'>スライドのテキストとビジュアルを生成するAIエンジンを選択してください。</p>
            </div>
            {/* Text Provider */}
            <div className='p-3 border border-[#EDEEEF] rounded-[11px] '>
                <div className="flex items-center gap-6  mb-7">
                    ...
                    <div className='w-full'>
                        ...
                        <h3 className="text-xl font-normal text-[#191919] pb-1.5">テキスト生成設定</h3>
                ...
                <div className='flex items-start gap-4 '>
                    <div className="flex flex-col justify-start w-full ">
                        ...
                        >
                            ...
                            >
                                <Command>
                                    <CommandInput placeholder="プロバイダーを検索..." />
                    ...
                    <div className="relative flex flex-col justify-end  items-end  w-full ">
                        <div className="flex flex-col justify-start w-full ">
                            {llmConfig.LLM === 'ollama' ? (
                                ...
                                <>
                                    <label className="block text-sm font-medium capitalize text-gray-700 mb-2">
                                        {llmConfig.LLM === 'custom' ? 'カスタムLLM APIキー' : `${llmConfig.LLM} APIキー`}
                                    </label>
                                    <div className="relative">
                                        <input
                                            ...
                                            className="w-full px-2 py-3 outline-none border  border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
                                            placeholder={`${llmConfig.LLM} APIキーを入力`}
                                        />
                                        <button
                                            type="button"
                ...
                <div className='flex items-start gap-4 mt-4'>
                    ...
                    {modelsChecked && availableModels.length > 0 && (
                        <div className="w-full">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                    {llmConfig.LLM === 'ollama' ? '対応モデルを選択' : `${LLM_PROVIDERS[llmConfig.LLM!]?.label} モデルを選択`}
                                </label>
                                <div className="w-full">
                                    <Popover
            ...
            <div className='p-3 border border-[#EDEEEF] rounded-[11px] mt-5'>
                <ToolTip content="画像生成を有効/無効" className='flex justify-end items-center'>
                ...
                <div className=" mb-7 flex items-center gap-6">
                    ...
                    <div>
                        ...
                        <h3 className="text-xl font-normal text-[#191919] ">画像生成設定</h3>
                        <p className=" text-sm  text-gray-500">
                            画像のソースを選択
                        </p>
                    </div>
                                ...
                                return (
                                    <div className="w-full ">
                                        ...
                                        <div className="relative">
                                            <input
                                                type={showApiKey ? 'text' : 'password'}
                                                    placeholder={`${provider.apiKeyFieldLabel}を入力`}
                ...
                {!llmConfig.DISABLE_IMAGE_GENERATION && <div className='flex justify-end items-center mt-[18px]'>
                    ...
                    {llmConfig.IMAGE_PROVIDER === "comfyui" && <div className='w-full'>
                        ...
                        <div className="relative">
                            <textarea
                                placeholder='ComfyUIワークフローJSONをここに貼り付け（ComfyUIの「Export (API)」からエクスポート）'
                    ...
                    <div className="bg-white/95 backdrop-blur-md rounded-xl shadow-2xl max-w-md w-full p-6 relative">
                        ...
                        <div className="text-center">
                            ...
                            <h3 className="text-lg font-semibold text-gray-900 mb-2">
                                {downloadingModel.done ? "ダウンロード完了！" : "モデルをダウンロード中"}
                            </h3>
                            ...
                            {downloadingModel.status && downloadingModel.status !== "pulled" && (
                                <div className="text-xs text-gray-500">
                                    {downloadingModel.status === "downloading" && "モデルファイルをダウンロード中..."}
                                    {downloadingModel.status === "verifying" && "モデルの整合性を確認中..."}
                                    {downloadingModel.status === "pulling" && "レジストリからモデルを取得中..."}
                                </div>
                            )}
                            ...
                            {downloadingModel.downloaded && downloadingModel.size && (
                                <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                                    <div className="flex justify-between text-xs text-gray-600">
                                        <span>ダウンロード済み: {(downloadingModel.downloaded / 1024 / 1024).toFixed(1)} MB</span>
                                        <span>合計: {(downloadingModel.size / 1024 / 1024).toFixed(1)} MB</span>
                                    </div>
                                </div>
                            )}
                        </div>

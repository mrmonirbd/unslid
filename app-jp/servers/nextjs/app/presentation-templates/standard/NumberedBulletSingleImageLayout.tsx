...
const Schema = z.object({
  ...
  leftBullets: z.array(
    z.object({
      ...
      title: z.string().min(10).max(36).default("戦略的実行").meta({
        description: "Bullet title text. Designed for 24px. Max ~36 chars",
      }),
      body: z.string().min(60).max(100).default("500名以上の従業員を持つ企業に注力し、テクノロジーセクターを優先します。").meta({
  ...
  ).min(1).max(4).default([
    {
      numberText: "01",
      title: "戦略的実行",
      body: "500名以上の従業員を持つ企業に注力し、テクノロジーセクターを優先します。",
    },
    {
      numberText: "02",
      title: "強いチームの構築",
  ...
  rightHeader: z.object({
    heading: z.string().min(6).max(30).default("私たちの歩み").meta({
      description: "Right column heading. Max ~30 chars",
    }),
    paragraph: z.string().min(80).max(200).default("金融サービス、ヘルスケア、テクノロジーセクターで500名以上の従業員を持つ企業に注力。アカウントベースのマーケティングとコンテンツ主導の戦略により新規パイプラインを目標とします。").meta({
      description: "Right paragraph text. Max 200 chars",
    }),
  }).default({
    heading: "私たちの歩み",
    paragraph: "金融サービス、ヘルスケア、テクノロジーセクターで500名以上の従業員を持つ企業に注力。アカウントベースのマーケティングとコンテンツ主導の戦略により新規パイプラインを目標とします。",
  }),
})

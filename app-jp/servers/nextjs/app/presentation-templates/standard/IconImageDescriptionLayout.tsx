...
const Schema = z.object({
  ...
  title: z.string().min(24).max(56).default("アイデアを現実に変える").meta({
  ...
  card: z.object({
    ...
    heading: z.string().min(22).max(70).default("アイデアの創出と検証").meta({
      description: "Card heading text. Max 10 words",
    }),
    body: z.string().min(140).max(450).default("金融サービス、ヘルスケア、テクノロジーセクターで500名以上の従業員を持つ企業に注力。アカウントベースのマーケティングとコンテンツ主導の戦略により、CACを$150以下に抑えながら$3.5Mの新規パイプラインを目標とします。これにより持続的な成長と顧客価値の最大化を実現します。").meta({
    ...
    heading: "アイデアの創出と検証",
    body: "金融サービス、ヘルスケア、テクノロジーセクターで500名以上の従業員を持つ企業に注力。アカウントベースのマーケティングとコンテンツ主導の戦略により、CACを$150以下に抑えながら$3.5Mの新規パイプラインを目標とします。これにより持続的な成長と顧客価値の最大化を実現します。",
    image: {
      __image_url__: "https://via.placeholder.com/1200x600",
      __image_prompt__: "Optional supporting image inside card area",
    },

...
const Schema = z.object({
  ...
  heading: z.string().min(16).max(38).default("成功への青写真").meta({
    description: "Main heading across up to 2 lines. Max 7 words",
  }),
  subheading: z.string().min(60).max(200).default("金融サービス、ヘルスケア、テクノロジーセクターで500名以上の従業員を持つ企業に注力。アカウントベースのマーケティングとコンテンツ主導の戦略により、新規パイプラインを目標とします。").meta({
  ...
  cards: z.array(z.object({
    title: z.string().min(8).max(16).default("戦略 01").meta({
      description: "Card ribbon title. Max 3 words",
    }),
    body: z.string().min(60).max(160).default("500名以上の従業員を持つ企業に注力し、テクノロジーセクターを優先します。").meta({
      description: "Card body text. Max 28 words",
    }),
  })).min(1).max(4).default([
    { title: "戦略 01", body: "500名以上の従業員を持つ企業に注力し、テクノロジーセクターを優先します。" },
    { title: "戦略 02", body: "500名以上の従業員を持つ企業に注力し、テクノロジーセクターを優先します。" },
    { title: "戦略 03", body: "500名以上の従業員を持つ企業に注力し、テクノロジーセクターを優先します。" },
    { title: "戦略 04", body: "500名以上の従業員を持つ企業に注力し、テクノロジーセクターを優先します。" },
  ]).meta({

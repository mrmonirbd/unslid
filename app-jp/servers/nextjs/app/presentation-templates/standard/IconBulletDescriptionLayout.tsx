...
const Schema = z.object({
  ...
  rightTitle: z.string().min(24).max(72).default("業界を革新する").meta({
    description: "Large heading on the right. Max 8 words",
  }),
  rightDescription: z.string().min(120).max(240).default("金融サービス、ヘルスケア、テクノロジーセクターで500名以上の従業員を持つ企業に注力。アカウントベースのマーケティングとコンテンツ主導の戦略により、新規パイプラインを目標とします。").meta({
  ...
  cards: z.array(z.object({
    ...
    title: z.string().min(16).max(38).default("ビジョナリーなリーダーシップ").meta({
      description: "Title for the card item. Max 4 words",
    }),
    description: z.string().min(50).max(100).default("500名以上の従業員を持つ企業に注力し、テクノロジーセクターを優先します。").meta({
  ...
  })).min(1).max(4).default([
    {
      ...
      title: "ビジョナリーなリーダーシップ",
      description: "500名以上の従業員を持つ企業に注力し、テクノロジーセクターを優先します。",
    },
    {
      symbolText: "i",

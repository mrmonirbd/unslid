...
export const Schema = z.object({
    title: z.string().min(3).max(50).default('データ分析ダッシュボード'),
    description: z.string().min(10).max(200).default('複数のデータ次元にわたる主要指標とパフォーマンス指標の総合的な概要。'),
    metrics: z.array(z.object({
        value: z.string().max(12).default('$3.5M'),
        label: z.string().max(24).default('パイプライン'),
    })).max(4).default([
    ...
    charts: z.array(ChartItemSchema).min(1).max(6).default([
        { title: '四半期別収益', type: 'bar-vertical', data: [{ name: 'Q1', value: 125000 }, { name: 'Q2', value: 158000 }, { name: 'Q3', value: 142000 }, { name: 'Q4', value: 189000 }], colorPalette: 'vibrant' },
        { title: '市場分布', type: 'donut', data: [{ name: '北米', value: 35 }, { name: 'ヨーロッパ', value: 28 }, { name: 'アジア太平洋', value: 25 }, { name: 'その他', value: 12 }], colorPalette: 'ocean' },
        { title: '成長トレンド', type: 'line', data: [{ name: '1月', value: 30 }, { name: '2月', value: 45 }, { name: '3月', value: 52 }, { name: '4月', value: 48 }, { name: '5月', value: 67 }, { name: '6月', value: 82 }], colorPalette: 'professional' },
        { title: '部門別パフォーマンス', type: 'bar-horizontal', data: [{ name: '営業', value: 87 }, { name: 'マーケティング', value: 72 }, { name: 'エンジニアリング', value: 95 }, { name: 'サポート', value: 68 }], colorPalette: 'sunset' },
        { title: '製品比較', type: 'bar-clustered', data: [{ name: 'Q1', values: { '製品A': 45, '製品B': 62 } }, { name: 'Q2', values: { '製品A': 58, '製品B': 71 } }, { name: 'Q3', values: { '製品A': 72, '製品B': 65 } }], series: ['製品A', '製品B'], colorPalette: 'vibrant' },
        { title: '顧客フィードバック', type: 'bar-diverging', data: [{ name: '品質', positive: 78, negative: 22 }, { name: 'サービス', positive: 65, negative: 35 }, { name: '価格', positive: 42, negative: 58 }], series: ['満足', '不満'], colorPalette: 'professional' },
    ]),
...
const MiniChartRenderer: React.FC<{
    ...
    switch (chart.type) {
        ...
        default:
            return <div className="flex items-center justify-center h-full text-gray-400 text-sm">サポートされていないグラフタイプ</div>;
    }
};
...
const TitleDescriptionMultiChartGridWithMetricsLayout: React.FC<MultiChartGridSlideLayoutProps> = ({ data: slideData }) => {
    const title = slideData?.title || 'データ分析ダッシュボード';

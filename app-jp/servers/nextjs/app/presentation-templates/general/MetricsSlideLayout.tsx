...
const metricsSlideSchema = z.object({
    title: z.string().min(3).max(100).default('会社の実績').meta({
    ...
    metrics: z.array(z.object({
    ...
    })).min(2).max(3).default([
        {
            value: '150+',
            label: '顧客数',
            description: 'Larana Inc.は多様な顧客基盤の構築に成功し、業界全体で信頼を獲得しています。'
        },
        {
            value: '200+',
            label: '完了プロジェクト数',
            description: 'Larana Inc.は200件以上のプロジェクトを完遂し、常に変化するクライアントのニーズに対応しています。'
        },
        {
            value: '95%',
            label: '顧客満足度',
            description: 'Larana Inc.は顧客の成功に重点を置き、95%の満足率を達成しています。'
        }
    ]).meta({

...
const introSlideSchema = z.object({
    title: z.string().min(3).max(40).default('製品概要').meta({
        description: "Main title of the slide",
    }),
    description: z.string().min(10).max(150).default('私たちの製品は、リアルタイムレポートとデータドリブンな意思決定のためのカスタマイズ可能なダッシュボードを提供します。サードパーティツールとの統合により業務を強化し、ビジネスの成長に合わせてスケールし、効率性を向上させます。').meta({
        description: "Main description text content",
    }),
    presenterName: z.string().min(2).max(50).default('John Doe').meta({
        description: "Name of the presenter",
    }),
    presentationDate: z.string().min(2).max(50).default('2025年12月').meta({
                ...
                <div className="relative z-10 flex h-full px-8 sm:px-12 lg:px-20 pt-12 pb-8">
                    ...
                    <div className="flex-1 flex flex-col justify-center pl-8 space-y-6">
                        ...
                        <p style={{ color: "var(--background-text,#4b5563)" }} className="text-base sm:text-lg text-gray-700 leading-relaxed">
                            {slideData?.description || '私たちの製品は、リアルタイムレポートとデータドリブンな意思決定のためのカスタマイズ可能なダッシュボードを提供します。サードパーティツールとの統合により業務を強化し、ビジネスの成長に合わせてスケールし、効率性を向上させます。'}
                        </p>

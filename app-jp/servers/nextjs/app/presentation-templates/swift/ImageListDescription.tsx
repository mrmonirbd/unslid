...
const ItemSchema = z
  .object({
    title: z.string().min(2).max(40).default("サンプルタイトル"),
    description: z
      .string()
      .min(10)
      .max(140)
      .default("画像またはアイテムの短い説明。"),
    image: ImageSchema,
  })
  .default({
    title: "サンプルタイトル",
    description: "画像またはアイテムの短い説明。",
    image: ImageSchema.parse({}),
  })

const Schema = z
  .object({
    titleLine1: z.string().min(3).max(24).default("私たちの"),
    titleLine2: z.string().min(3).max(24).default("チーム"),
    description: z
      .string()
      .min(20)
      .max(200)
      .default(
        "金融サービス、ヘルスケア、テクノロジーセクターで500名以上の従業員を持つ企業に注力。"
      ),
    items: z
      .array(ItemSchema)
      .min(3)
      ...
      .default([
        ItemSchema.parse({}),
        ItemSchema.parse({ title: "アイテム2", description: "簡潔なサポートテキスト。", image: ImageSchema.parse({}) }),
        ItemSchema.parse({ title: "アイテム3", description: "簡潔なサポートテキスト。", image: ImageSchema.parse({}) }),
      ]),

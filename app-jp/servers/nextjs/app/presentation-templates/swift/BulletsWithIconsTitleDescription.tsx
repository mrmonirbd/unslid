...
const ItemSchema = z
  .object({
    icon: IconSchema,
    title: z.string().min(3).max(40).default("概要テキスト"),
    description: z
      .string()
      .min(0)
      .max(160)
      .default(
        "アイコンタイトルの下に収まる簡潔な説明テキスト。"
      ),
  })
  .default({
    icon: {
    ...
    description: "アイコンタイトルの下に収まる簡潔な説明テキスト。",
  })
...
const Schema = z
  .object({
    title: z
      ...
      .max(60)
      .default("インフォグラフィック"),
    sideHeading: z.string().min(0).max(60).default("左側の見出しテキスト"),
    sideParagraph: z
      .string()
      .min(0)
      .max(300)
      .default(
        "文脈を説明する簡潔な段落。1〜2行で読みやすく保ってください。"
      ),
    items: z
      .array(ItemSchema)
      .min(3)

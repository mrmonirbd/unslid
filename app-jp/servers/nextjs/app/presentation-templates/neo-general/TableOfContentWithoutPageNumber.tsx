...
export const Schema = z.object({
    title: z.string().max(30).describe('The main heading of the slide').default('目次'),
    items: z.array(z.object({
        number: z.string().max(5).describe('Sequence number or index').default('1'),
        label: z.string().max(40).describe('Label text for the item').default('はじめに'),
    })).describe('List of items displayed in two columns').default([
        { number: '1', label: 'はじめに' },
        { number: '2', label: '主な発見' },
        { number: '3', label: 'データ分析' },
        { number: '4', label: '推奨事項' },
        { number: '5', label: 'まとめ' },
        { number: '6', label: 'はじめに' },
        { number: '7', label: '主な発見' },
        { number: '8', label: 'データ分析' },
        { number: '9', label: '推奨事項' },
        { number: '10', label: 'まとめ' },
    ]),
});

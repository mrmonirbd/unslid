...
export const Schema = z.object({
    title: z.string().max(30).describe('The main heading of the slide').default('顧客事例 / ケーススナップショット'),
    challengeSectionTitle: z.string().max(12).describe('Heading for the first content section').default('課題'),
    challengeContent: z.string().max(140).describe('Descriptive text for the first section').default('12地域にまたがるマーケティング業務の分断により、支出配分の非効率と一貫性のないメッセージングが生じました。CACは前年比43%増加しました。'),
    outcomeSectionTitle: z.string().max(12).describe('Heading for the second content section').default('成果'),
    ...
    customerSubTitle: z.string().max(26).describe('Subtitle or secondary text in the card').default('Fortune 500テクノロジー企業'),
    metricValue: z.string().max(6).describe('The primary metric or statistic value').default('$4.2M'),
    metricLabel: z.string().max(26).describe('Label describing the metric').default('Q4の追加パイプライン'),
    metricIcon: z.object({

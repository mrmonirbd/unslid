...
export const Schema = z.object({
    title: z.string().max(20).describe('The main heading of the slide').default('リスクと制約'),
    items: z.array(z.object({
        bgTitle: z.string().max(10).describe('Large category label displayed prominently').default('市場'),
        subtitle: z.string().max(20).describe('Secondary heading for the item').default('市場飽和'),
        description: z.string().max(70).describe('Detailed description text for the item').default('主要分野での競争激化により、コンバージョン率とCACに圧力がかかる可能性があります')
    })).max(3).describe('List of category items with details').default([
        {
            bgTitle: '市場',
            subtitle: '市場飽和',
            description: '主要分野での競争激化により、コンバージョン率とCACに圧力がかかる可能性があります'
        },
        {
            bgTitle: '予算',
            subtitle: '予算制約',
            description: 'Q1の予算15%削減により、成功したキャンペーンの拡大能力が制限される可能性があります'
        },
        {
            bgTitle: '資源',
            subtitle: 'リソース容量',
            description: 'コンテンツ制作チームが110%稼働中でコンテンツ速度に影響する可能性があります'
        }
    ])
});

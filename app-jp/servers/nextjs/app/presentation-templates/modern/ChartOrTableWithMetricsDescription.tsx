...
const tractionSchema = z.object({
  ...
  title: z.string().default("会社の実績").meta({
  ...
  description: z
    ...
    .max(200)
    .default(
      "トラクションとは、会社が開発期間中に勢いを感じる期間です。トラクションの勢いを活かさなければ、売上が減少し、顧客基盤が縮小する可能性があります。一般的に、企業は収益と新規顧客の数によって成功を評価します。",
    )
    .meta({
      description:
  ...
  tableColumns: z.array(z.string().min(1).max(40)).min(2).max(10).default(["指標", "値"]),
  tableRows: z.array(z.array(z.string().min(0).max(200)).min(2).max(10)).min(1).max(30).default([["ユーザー数", "10K+"], ["収益", "$1.2M"], ["満足度", "95%"]]),

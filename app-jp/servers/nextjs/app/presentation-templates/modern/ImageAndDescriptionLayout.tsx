...
const imageWithDescriptionSlideSchema = z.object({
  title: z.string().min(3).max(30).default("画像と説明").meta({
  ...
  content: z
    ...
    .max(300)
    .default(
      "プレゼンテーションセッションでは、背景・はじめにには、プレゼンテーションの冒頭での議論の材料として興味深いトピックについて、体系的かつ効果的に整理された情報を記入することができます。はじめには、発表を聞いている方々に全体的な概要を提供し、この背景・はじめにのセッションで議題のキーワードを強調します。",
    )
    .meta({

...
const SupportingDoc = ({
    ...
    const handleValidate = (filesToReview: File[]) => {
        ...
        if (disallowed.length > 0) {
            toast.error('対応していないファイルがあります', {
                description: 'PDF、TXT、PPTX、DOCXファイルのみ対応しています。',
            })
        }
    }
    ...
    const handleFilesSelected = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (allowedFiles.length > files.length) {
            toast.success('ファイルが選択されました', {
                description: `${allowedFiles.length - files.length}件のファイルが追加されました`,
            })
        }
        ...
        <div className="space-y-2" data-testid="attachments-uploader">
            <div className="flex items-center justify-between">
                <p className="text-sm text-gray-600 font-syne">
                    {hasFiles ? `${filteredFiles.length}件の添付ファイル` : '添付ファイルなし'}
                </p>
                <button
                    type="button"
            ...
            >
                ...
                <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm font-medium text-gray-800 font-syne">
                        PDF、TXT、PPTX、DOCXをドラッグ＆ドロップするか、<span className="text-[#5146E5]">クリックして選択</span>
                    </p>
                </div>
            </label>
            ...
            {hasFiles && (
                <div className="mt-2">
                    <ul data-testid="file-list" className="grid grid-cols-1 gap-2 sm:grid-cols-2" aria-label="添付ファイル">
                    ...
                    {filteredFiles.length !== files.length && (
                        <p className="mt-2 text-xs text-amber-600 font-syne">
                            一部のファイルはスキップされました。PDF、TXT、PPTX、DOCXファイルのみ対応しています。
                        </p>
                    )}
                </div>
            )}

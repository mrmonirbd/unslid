...
const PresentationHeader = ({
  ...
  const RATIOS = [
    { key: "16:9", label: "ワイドスクリーン 16:9", desc: "標準プレゼン" },
    { key: "4:3", label: "クラシック 4:3", desc: "従来のスライド" },
    { key: "9:16", label: "縦向き 9:16", desc: "モバイル / SNS" },
    { key: "1:1", label: "スクエア 1:1", desc: "LinkedIn / Instagram" },
    { key: "A4", label: "A4 縦", desc: "ドキュメント / 印刷用" },
  ];
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        toast.error(e?.message || "テーマの読み込みに失敗しました");
      }
    };
    if (themes.length === 0) {
      load();
  ...
  const handleRestoreVersion = async (versionId: number) => {
    if (!confirm("このバージョンを復元しますか？現在のスライドの変更は上書きされます。")) return;
    ...
    try {
      ...
      toast.success("バージョンを復元しました — 再読み込み中…");
  ...
  const handleExportPptx = async () => {
    ...
    try {
      ...
      if (!pptx_model) throw new Error("PPTXモデルの構築に失敗しました");
      ...
      toast.error("エクスポートに問題が発生しました！", {
        description: "プレゼンのエクスポートに問題が発生しました。もう一度お試しください。",
      });
    } finally {
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error((err as any).error || "PDFのエクスポートに失敗しました");
      }
      ...
      toast.error("エクスポートに問題が発生しました！", {
        description:
          "プレゼンのエクスポートに問題が発生しました。もう一度お試しください。",
      });
    } finally {
      setIsExporting(false);
    }
  ...
  const ExportOptions = ({ mobile }: { mobile: boolean }) => (
    <div className={` rounded-[18px] max-md:mt-4 ${mobile ? "" : "bg-white"}  p-5`}>
      <p className="text-sm font-medium text-[#19001F]">エクスポート形式</p>
          ...
          <div className="w-80 bg-white h-full flex flex-col shadow-2xl border-l border-slate-200">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-slate-800 text-sm">バージョン履歴</span>
              </div>
            ...
            <div className="px-4 py-3 border-b border-slate-100">
              ...
              >
                ...
                {savingVersion ? "保存中…" : "現在のバージョンを保存"}
              </button>
            </div>

            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
              {versionsLoading && <p className="text-sm text-slate-400 text-center py-4">読み込み中…</p>}
              {!versionsLoading && versions.length === 0 && (
                <p className="text-sm text-slate-400 text-center py-4">保存済みのバージョンがありません。</p>
              )}
              {versions.map((v) => (
      ...
      <div className="py-7 sticky top-0 bg-white z-50 mb-[17px]  font-syne flex justify-between items-center">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <h2 className="text-lg text-[#101323] font-unbounded min-w-0">
            <MarkdownRenderer content={presentationData?.title || "プレゼン"} className="mb-0 max-w-[400px] truncate text-sm text-[#101323]" />
          </h2>
        </div>
        <div className="flex items-center gap-2.5">
          ...
          <ToolTip content="バージョン履歴">
            ...
            >
              ...
              <span className="hidden lg:inline">履歴</span>
            </button>
          </ToolTip>
          ...
          <Popover open={resizeOpen} onOpenChange={setResizeOpen}>
            ...
            <PopoverContent align="end" className="w-[220px] p-2 rounded-xl shadow-xl border border-slate-200">
              <p className="text-xs font-semibold text-slate-500 px-2 py-1 uppercase tracking-widest">アスペクト比</p>
              {RATIOS.map((r) => (
                <button
                  key={r.key}
          ...
          <Popover open={shareOpen} onOpenChange={handleShareOpen}>
            ...
            <PopoverContent align="end" className="w-[320px] p-0 rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
              ...
              <div className="px-5 py-4 border-b border-slate-100">
                <p className="font-semibold text-slate-900 text-sm">プレゼンを共有</p>
                <p className="text-xs text-slate-500 mt-0.5">
                  {shareUrl ? "共有リンク（有効）" : "未共有 — 以下でアクセスタイプを選択"}
                </p>
              </div>
              ...
              <div className="px-5 py-4 space-y-4">
                ...
                <div>
                  <p className="text-xs font-medium text-slate-600 mb-2">
                    {shareUrl ? "アクセスタイプを変更" : "アクセスタイプを選択"}
                  </p>
                ...
                {shareMode === "password" && (
                  <div>
                    <label className="block text-xs font-medium text-slate-600 mb-1.5">閲覧者用パスワード</label>
                    <input
                      type="password"
                      value={sharePassword}
                ...
                >
                  {shareLoading ? (
                    ...
                    <>
                      <Share2 className="w-4 h-4" />
                      {shareMode === "password" ? "保護されたリンクを作成" : "共有リンクを作成"}
                    </>
                  )}
                </button>
              </div>
          ...
          <Popover open={open} onOpenChange={setOpen} >
            <PopoverTrigger asChild>
              ...
              >
                {isExporting ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : "エクスポート"} <ArrowRightFromLine className="w-3.5 h-3.5" />
              </button>
            </PopoverTrigger>

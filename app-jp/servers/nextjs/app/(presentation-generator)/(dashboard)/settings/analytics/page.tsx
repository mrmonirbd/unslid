  ...
  return (
    <div className="max-w-4xl mx-auto px-8 py-10 space-y-8">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-slate-900">共有分析</h1>
      </div>
      <p className="text-sm text-slate-500">
        共有プレゼンのビューワーのエンゲージメントを確認します。
      </p>

      {loading ? (
        <div className="text-slate-400 text-sm">共有リンクを読み込み中...</div>
      ) : shares.length === 0 ? (
        <div className="rounded-xl border border-dashed border-slate-300 p-10 text-center text-slate-400 text-sm">
          まだ共有されたプレゼンはありません。プレゼンを共有すると、ここで分析が表示されます。
        </div>
      ) : (
            ...
            {analytics && !analyticsLoading && (
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-3 gap-4">
                  {[
                    { label: "総閲覧数", value: analytics.total_views, icon: Eye },
                    { label: "ユニーク閲覧者", value: analytics.unique_viewers, icon: Users },
                    { label: "追跡スライド数", value: analytics.per_slide.length, icon: BarChart2 },
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-5">
                  <h3 className="text-sm font-semibold text-slate-700 mb-4">スライドごとの閲覧数</h3>
                  {analytics.per_slide.length === 0 ? (
                    <p className="text-sm text-slate-400">まだスライドデータがありません。閲覧者はスライドに少なくとも5秒滞在する必要があります。</p>
                  ) : (

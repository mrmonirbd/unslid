...
const PLAN_LABELS: Record<string, string> = { free: "無料", pro: "Pro", team: "チーム" };
...
export default function BillingPage() {
  ...
  const handleTrialCheckout = async () => {
    ...
    try {
      ...
      alert(err?.message || "試用チェックアウトを開始できませんでした。もう一度お試しください。");
    } finally {
  ...
  const handleCheckout = async (priceId: string, planKey: string) => {
    if (!priceId) {
      alert("請求はまだ設定されていません。サポートにお問い合わせいただくか、後でご確認ください。");
      return;
    }
  ...
  const handlePortal = async () => {
    ...
    try {
      ...
      alert("請求ポータルを開けませんでした。もう一度お試しください。");
    } finally {
  ...
  const formatPrice = (entry: PlanPricingEntry) => {
    if (entry.price_monthly === 0) return "無料";
  ...
  return (
    <div className="max-w-3xl mx-auto px-8 py-10 space-y-8">
      <h1 className="text-2xl font-bold text-slate-900">請求</h1>
      ...
      {trialSuccess && status?.trial_days_remaining != null && (
        <div className="p-4 bg-amber-50 border border-amber-300 rounded-xl text-amber-800 text-sm font-medium flex items-center gap-3">
          ...
          <span>
            <strong>{status.trial_config?.name ?? "Spark"} が有効になりました！</strong> フルProアクセスが{" "}
            <strong>{status.trial_days_remaining}日間</strong>使えます。期限が切れる前にProにアップグレードしてすべてを維持してください。
          </span>
        </div>
      )}
      {success && (
        <div className="p-4 bg-green-50 border border-green-300 rounded-xl text-green-800 text-sm font-medium flex items-center gap-2">
          <Check className="h-4 w-4" /> サブスクリプションが有効になりました。{PLAN_LABELS[currentPlan]}プランへようこそ！
        </div>
      )}
      {cancelled && (
        <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl text-amber-800 text-sm">
          チェックアウトがキャンセルされました。プランは変更されていません。
        </div>
      )}
      {status?.subscription_status === "past_due" && (
        <div className="p-4 bg-red-50 border border-red-300 rounded-xl text-red-800 text-sm flex items-start gap-3">
          ...
          <div>
            <p className="font-semibold">支払いに失敗しました — 対応が必要です</p>
            <p className="mt-0.5">前回のお支払いを処理できませんでした。プランへのアクセスを失わないよう、支払い方法を更新してください。</p>
            <button onClick={handlePortal} className="mt-2 underline font-medium">支払い方法を更新 →</button>
          </div>
        </div>
      )}
      {status?.cancel_at_period_end && status.subscription_ends_at && (
        <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl text-amber-800 text-sm flex items-start gap-3">
          <AlertTriangle className="h-4 w-4 mt-0.5 shrink-0" />
          <div>
            <p className="font-semibold">サブスクリプション終了</p>
            <p className="mt-0.5">
              サブスクリプションは{" "}
              <strong>{new Date(status.subscription_ends_at).toLocaleDateString()}</strong>に終了します。その後、アカウントは無料プランに移行します。
            </p>
            <button onClick={handlePortal} className="mt-2 underline font-medium">再開 →</button>
          </div>
        </div>
      )}
        ...
        <div className={`p-4 border rounded-xl text-sm flex items-start gap-3 ${
          ...
          <div>
            <p className="font-semibold">
              {status.trial_config?.name ?? "Spark"} 試用期間 —{" "}
              {status.trial_days_remaining === 0
                ? "本日期限切れ"
                : `あと${status.trial_days_remaining}日`}
            </p>
            <p className="mt-0.5">
              試用期間は{" "}
              <strong>{new Date(status.trial_expires_at).toLocaleDateString()}</strong>に終了します。
              Proにアップグレードして、中断なくアクセスとプレゼンを維持してください。
            </p>
          </div>
        </div>
      )}
      ...
      <section className="bg-white border border-slate-200 rounded-xl p-6 space-y-4">
        <h2 className="font-semibold text-slate-800">現在のプラン</h2>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {currentPlan === "free" && (
              <span className="text-sm text-slate-500">永久無料</span>
            )}
          </div>
          {status?.has_billing && (
            <button
            ...
            >
              {portalLoading ? "読み込み中..." : "サブスクリプションを管理"}
            </button>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 pt-2">
          <div className="bg-slate-50 rounded-lg p-3">
            <p className="text-xs text-slate-500">今月のプレゼン数</p>
          ...
          <div className="bg-slate-50 rounded-lg p-3">
            <p className="text-xs text-slate-500">使用ストレージ</p>
            <p className="text-lg font-semibold text-slate-800 mt-0.5">
      ...
      {currentPlan === "free" && pricing && (
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-slate-800">プランをアップグレード</h2>
            ...
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 border-2 border-amber-300 rounded-xl p-6 space-y-5 relative">
              ...
              <div>
                ...
                <div className="flex items-end gap-1">
                  ...
                  <span className="text-slate-500 text-sm mb-1">一回払い</span>
                </div>
                <p className="text-xs text-slate-500 mt-0.5">
                  {status.trial_config.days}日間のフルProアクセス — サブスクリプションなし
                </p>
              </div>
              ...
              >
                ...
                {checkoutLoading === "trial" ? "リダイレクト中…" : `${status.trial_config.name}アクセスを取得 — $${status.trial_config.price % 1 === 0 ? status.trial_config.price.toFixed(0) : status.trial_config.price.toFixed(2)}`}
              </button>
              <p className="text-center text-xs text-slate-400">
                一回払い。{status.trial_config.days}日後に自動的に終了します。サブスクリプション不要。
              </p>
            </div>
          )}
                ...
                >
                  ...
                  <div>
                    ...
                    {cadence === "annual" && entry.price_annual > 0 && (
                      <p className="text-xs text-slate-500 mt-0.5">
                        年額請求 (${entry.price_annual}/年)
                        {saving > 0 && <span className="ml-1 text-green-600 font-semibold">· {saving}%割引</span>}
                      </p>
                    )}
                    {cadence === "monthly" && entry.price_annual > 0 && saving > 0 && (
                      <p className="text-xs text-slate-400 mt-0.5">年額に切り替えると{saving}%割引</p>
                    )}
                  </div>
                  ...
                  >
                    {checkoutLoading === planKey
                      ? "リダイレクト中…"
                      : priceId
                        ? `${PLAN_LABELS[planKey]}にアップグレード`
                        : `${PLAN_LABELS[planKey]}にアップグレード（お問い合わせ）`}
                  </button>
                </div>
              );
            })}
      ...
      {currentPlan === "pro" && pricing && (
        <section className="bg-purple-50 border border-purple-200 rounded-xl p-6 space-y-4">
          <h2 className="font-semibold text-purple-800">チームで協力しますか？</h2>
          <p className="text-sm text-purple-700">
            <strong>チームプラン</strong>にアップグレードして同僚を招待し、プレゼンを共有・共同作業できます。既存のファイルはすべて引き継がれ、新しいチーム共有機能が使えます。
          </p>
          <div className="flex items-center gap-4">
            <div>
              <p className="text-xs text-purple-600">チームプランは</p>
              <p className="text-2xl font-bold text-purple-900">
                ${pricing.team.price_monthly}/月 <span className="text-sm font-normal">シートあたり</span>
              </p>
              <p className="text-xs text-purple-500">最低2シート</p>
            </div>
            <button
              onClick={() => {
                const priceId = cadence === "monthly"
            ...
            >
              {checkoutLoading === "team" ? "リダイレクト中…" : "チームにアップグレード →"}
            </button>
          </div>
          <p className="text-xs text-purple-500">Proサブスクリプションはキャンセルされ、置き換えられます。日割りクレジットが適用されます。</p>
        </section>
      )}
      ...
      {currentPlan !== "free" && pricing && (
        <section className="bg-white border border-slate-200 rounded-xl p-6 space-y-3">
          <h2 className="font-semibold text-slate-800">プランに含まれるもの</h2>
          ...
          {status?.has_billing && (
            <p className="text-xs text-slate-400 pt-2">
              <button onClick={handlePortal} className="underline text-indigo-500">請求ポータル</button>からサブスクリプションを管理またはキャンセルできます。
            </p>
          )}
        </section>
      )}
            ...
            <div className="flex items-center gap-3 bg-slate-50 rounded-lg p-3">
              ...
              <div>
                <p className="text-sm font-medium text-slate-800 capitalize">
                  {paymentMethod.brand}（末尾: {paymentMethod.last4}）
                </p>
                <p className="text-xs text-slate-500">
                  ...
                  (paymentMethod.exp_year === new Date().getFullYear() &&
                    ...
                    <span className="ml-2 text-red-500 font-semibold">期限切れ</span>
                  ) : null}
                </p>
              </div>
            </div>
          ) : (
            <p className="text-sm text-slate-400">登録された支払い方法がありません。</p>
          )}
        </section>
      )}

...
export const PresentationCard = ({
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (!confirm("このプレゼンを削除しますか？この操作は取り消せません。")) return;
    const response = await DashboardApi.deletePresentation(id);
    if (response) {
      toast.success("プレゼンを削除しました");
      if (onDeleted) onDeleted(id);
    } else {
      toast.error("プレゼンの削除に失敗しました");
    }
  };
  ...
  const handleToggleVisibility = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      setVisibility(next);
      toast.success(next === "team" ? "チームと共有しました" : "非公開に設定しました");
    } catch (err: any) {
      toast.error(err?.message || "共有設定を更新できませんでした");
    } finally {
  ...
  const updatedDate = presentation?.updated_at
    ? new Date(presentation.updated_at).toLocaleDateString("ja-JP", { month: "short", day: "numeric", year: "numeric" })
    : presentation?.created_at
    ? new Date(presentation.created_at).toLocaleDateString("ja-JP", { month: "short", day: "numeric", year: "numeric" })
    : "—";

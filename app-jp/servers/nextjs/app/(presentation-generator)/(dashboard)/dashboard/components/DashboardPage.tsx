...
const DashboardPage: React.FC = () => {
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert("プレゼンのインポートに失敗しました。有効な.pptxファイルか確認してください。");
    } finally {
      setImporting(false);
  ...
  return (
    <div className="min-h-full px-8 pb-12">
      ...
      <div className="pt-8 pb-6">
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 max-w-lg">
          <div className="bg-white rounded-xl border border-slate-200 px-4 py-3 shadow-sm">
            ...
            <p className="text-xs text-slate-400 mt-0.5">プレゼン</p>
          </div>

          <div className="bg-white rounded-xl border border-slate-200 px-4 py-3 shadow-sm">
            <div className="flex items-center gap-2 mb-1">
              <TrendingUp className="w-4 h-4 text-emerald-500" />
              <span className="text-xs font-medium text-slate-500">今月</span>
            </div>
            <p className="text-2xl font-bold text-slate-900">{thisMonth}</p>
            <p className="text-xs text-slate-400 mt-0.5">
              {monthlyLimit ? `${monthlyLimit}件まで` : "無制限"}
            </p>
          </div>
            ...
            <div className="bg-white rounded-xl border border-indigo-100 px-4 py-3 shadow-sm col-span-2 sm:col-span-1">
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-slate-500">使用量</span>
                </div>
                <Link
                  href="/settings/billing"
                  className="text-[11px] font-semibold text-indigo-600 hover:text-indigo-800"
                >
                  {usagePct >= 100 ? "アップグレード →" : "アップグレード"}
                </Link>
              </div>
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-semibold text-slate-700">最近のプレゼン</h2>
        <span className="text-xs text-slate-400">{!isLoading && totalCount > 0 ? `合計 ${totalCount} 件` : ""}</span>
      </div>

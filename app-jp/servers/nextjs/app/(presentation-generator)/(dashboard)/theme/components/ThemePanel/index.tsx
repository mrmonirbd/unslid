...
const ThemePanel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadCustomThemes = async () => {
      try {
        ...
        toast.error(error?.message || 'カスタムテーマの読み込みに失敗しました')
      }
    }
    const loadUserFonts = async () => {
      try {
        ...
        toast.error(error?.message || 'フォントの読み込みに失敗しました')
      }
    }
    loadUserFonts()
  ...
  const saveAsCustom = async () => {
    ...
    if (selectedTheme.user && selectedTheme.user !== 'system' && !selectedTheme.id.startsWith('custom-')) {
      ; (async () => {
        try {
          ...
          toast.error(error?.message || 'テーマの更新に失敗しました')
        }
      })()
      return
    }
    try {
      const params: ThemeParams = {
        name: selectedTheme.name,
        description: selectedTheme.description || `${selectedTheme.name}のカスタム版`,
        logo: customBrandLogoId || null,
        logo_url: customBrandLogo || null,
        company_name: themeCompanyName || null,
        data: {
      ...
      toast.error(error?.message || 'テーマの保存に失敗しました')
    }
  }
  ...
  const handleCustomFontChange = async (fontFile: File) => {
    try {
      ...
      toast.success(`フォント「${name}」をアップロードしました`)
    } catch (error: any) {
      console.error('Failed to upload font', error)
      toast.error(error?.message || 'フォントのアップロードに失敗しました')
    } finally {
    ...
    >
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          {step === 2 && <p className="text-xs text-[#4C4C4C] font-medium rounded-lg text-end pb-1.5">ブランドカラー</p>}
          <div className="space-y-4"
            style={{
        ...
        {step === 2 && <div>
          <p className="text-xs text-[#4C4C4C] font-medium text-end pb-1.5">テキストカラー</p>
          <div className="space-y-4"
            style={{
              padding: step === 2 ? '10px' : '0px',
              backgroundColor: step === 2 ? '#F9FAFB' : 'transparent'
        ...
        {step === 2 && <div>
          <p className="text-xs text-[#4C4C4C] font-medium text-end pb-1.5">グラフ/チャートカラー</p>
          <div className="space-y-4"
            style={{
              padding: step === 2 ? '10px' : '0px',
              backgroundColor: step === 2 ? '#F9FAFB' : 'transparent'
  ...
  const renderFontStep = () => (
    ...
    >
      ...
      <div className="px-2.5">
        <p className='text-xs text-[#4C4C4C] font-medium text-end pb-1.5'>カスタムフォントをアップロード</p>
        <div
          ...
          {isFontUploading ? (
            <div className='flex items-center gap-3'>
              ...
              <div className='flex-1'>
                <p className='text-sm font-medium text-[#7A5AF8]'>フォントをアップロード中...</p>
                <p className='text-xs text-[#888]'>しばらくお待ちください</p>
              </div>
            </div>
          ) : (
            <div className='flex items-center gap-3'>
              ...
              <div className='flex-1'>
                <p className='text-sm font-medium text-[#151515]'>フォントファイルをアップロード</p>
      ...
      {userFonts.fonts.length > 0 && (
        <div className="px-2.5">
          <p className='text-xs text-[#4C4C4C] font-medium text-end pb-1.5'>アップロード済みフォント</p>
      ...
      {/* Preset Fonts */}
      <div className='px-2.5'>
        <p className='text-xs text-[#4C4C4C] font-medium text-end pb-1.5'>プリセット</p>
        ...
        <div className="border-2 border-dashed  border-gray-300 rounded-lg p-6 text-center">
          {isLogoUploading ? (
            <div className="flex flex-col items-center justify-center py-6 text-gray-500">
              ...
              <p className="text-sm">ロゴをアップロード中...</p>
            </div>
          ) : customBrandLogo ? (
            <div className="space-y-2">
              <img
    ...
    <div className="space-y-6 px-6 font-syne">
      <div className='py-[28px] flex justify-between'>
        ...
        >
          ...
          <span className="hidden md:inline">新規テーマ</span>
                ...
                <div className='pt-7  w-full  h-[calc(90vh-120px)] flex flex-col justify-between  '>
                  ...
                  <div className='border-t border-[#EDEEEF] py-6 '>
                    <div className='flex justify-end px-5 gap-2'>
                      ...
                      >
                        {currentStep === 1 ? 'テーマパレットを生成' : currentStep === 2 ? 'フォントへ進む' : currentStep === 3 ? 'デザインへ進む' : 'カスタムテーマとして保存'}
                        <ChevronRight className='w-4 h-4' />
                      </button>
                    </div>
                  </div>

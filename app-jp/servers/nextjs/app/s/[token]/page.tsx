...
export default function ShareViewerPage({
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/v1/s/${token}`)
      .then((r) => r.json())
      .then(setShareData)
      .catch(() => setError("この共有リンクは無効か期限切れです。"))
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(err.detail ?? "パスワードが正しくありません");
        return;
      }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        <div className="text-slate-400 text-sm">プレゼンを読み込み中…</div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <h1 className="text-xl font-bold text-white mb-2">リンクが見つかりません</h1>
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8 w-full max-w-sm shadow-2xl">
          <div className="text-center mb-6">
            ...
            <h1 className="text-xl font-bold text-white">パスワード保護</h1>
            <p className="text-slate-400 text-sm mt-1">このプレゼンを表示するにはパスワードを入力してください。</p>
          </div>
          {error && (
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between">
        <div>
          <h1 className="font-bold text-slate-900">{presentation.title || "タイトルなしのプレゼン"}</h1>
          <p className="text-xs text-slate-500 mt-0.5">{presentation.n_slides} スライド</p>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-xs text-slate-400">{shareData?.view_count ?? 0} 回閲覧</span>
          <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-medium">読み取り専用</span>
        </div>
      </header>
        ...
        {presentation.structure ? (
          <div className="space-y-6">
            <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
              <p className="text-xs text-slate-400 mb-2 uppercase tracking-wide font-medium">プレゼンデータ</p>
              <p className="text-sm text-slate-700">
                {presentation.n_slides} スライド • {presentation.title}
              </p>
              <p className="text-xs text-slate-400 mt-2">
                スライドの完全表示にはアプリで開く必要があります。{" "}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {((presentation.structure as { slides: { title?: string; headline?: string }[] }).slides ?? []).map(
                  (slide, i) => (
                    ...
                    >
                      <p className="text-xs text-slate-400 mb-1">スライド {i + 1}</p>
                      <p className="font-medium text-slate-800 text-sm">
                        {slide.title ?? slide.headline ?? `スライド ${i + 1}`}
                      </p>
                    </div>
                  )
                )}

...
# Maximum size per uploaded document (20 MB)
...
async def upload_files(
    ...
    validate_files(files, True, True, 10, UPLOAD_ACCEPTED_FILE_TYPES)
    ...
    for each_file in files:
        ...
        if len(content) > MAX_FILE_SIZE_BYTES:
            raise HTTPException(
                status_code=413,
                detail=f"File '{each_file.filename}' exceeds the 20 MB limit.",
            )

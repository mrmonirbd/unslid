...
class LLMClient:
    ...
    def _get_anthropic_client(self):
        api_key = get_plan_config_value("ANTHROPIC_API_KEY", get_anthropic_api_key_env())
        if not api_key:
            raise HTTPException(
                status_code=400,
    ...
    def _get_ollama_client(self):
        base_url = get_plan_config_value("OLLAMA_URL", get_ollama_url_env()) or "http://localhost:11434"
        return AsyncOpenAI(
    ...
    def _get_custom_client(self):
        base_url = get_plan_config_value("CUSTOM_LLM_URL", get_custom_llm_url_env())
        if not base_url:
            raise HTTPException(
                status_code=400,
                detail="Custom LLM URL is not set",
            )
        api_key = get_plan_config_value("CUSTOM_LLM_API_KEY", get_custom_llm_api_key_env()) or "null"
        return AsyncOpenAI(
            base_url=base_url,
            api_key=api_key,
        )
    ...
    async def generate(
        ...
        # Track estimated token usage (non-blocking)
        try:
            from utils.plan_context import get_active_user_id
            from utils.rate_limit import track_token_usage
            import asyncio
            ...
            if uid:
                ...
                asyncio.create_task(track_token_usage(uid, input_chars + output_chars))
        except Exception:
            pass
        return content

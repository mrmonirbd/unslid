...
class PptxPresentationCreator:
    ...
    async def fetch_network_assets(self):
        ...
        def _process_image_path(each_shape, image_path):
            # Handle data URLs (base64-encoded images from client-side canvas extraction)
            if image_path.startswith("data:"):
                m = re.match(r"data:([^;]+);base64,(.+)", image_path, re.DOTALL)
                if m:
                    mime = m.group(1)
                    ...
                    if ext in ("jpeg", "jpg"):
                        ...
                        ext = "svg"
                    tmp_path = os.path.join(self._temp_dir, f"{_uuid.uuid4()}.{ext}")
                    with open(tmp_path, "wb") as fh:
                        fh.write(data)
                    each_shape.picture.path = tmp_path

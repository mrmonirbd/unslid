...
async def stream_outlines(
    ...
    async def inner():
        try:
            ...
            # Catch any unhandled exception inside the generator (e.g. missing API key
            # raised before the LLM stream starts) and emit it as an SSE error event
            # instead of crashing mid-stream.
            traceback.print_exc()
            detail = e.detail if isinstance(e, HTTPException) else str(e)
            yield SSEErrorResponse(detail=detail or "Outline generation failed. Please check AI model configuration.").to_string()
